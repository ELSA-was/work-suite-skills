---
name: today-watermark-camera-photo-assistant
...
7. 单个用户仍必须使用数组参数 `user_ids: ["xuser-..."]`，单个部门也必须使用 `department_ids: [...]`。
...
| `caller_user_id` | string | 是 | 当前登录用户 ID，必须来自上下文 |
...
|---|---|:---:|---|
| `group_id` | string | 是 | 团队短 ID |
| `begin_time` / `end_time` | string | 否 | 统计时间范围 |
| `user_ids` / `exclude_user_ids` | string[] | 否 | 包含或排除的用户 |
| `department_ids` | integer[] | 否 | 部门 ID 列表 |
...
| `watermark_name` | string | 否 | 水印模板名 |
| `page_size` | integer | 否 | 最大导出数量，默认 5000，最大 15000 |
