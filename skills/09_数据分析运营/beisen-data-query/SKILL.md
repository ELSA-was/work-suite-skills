...
> - `beisen-cli staffservice employeeData businessDataTool --params '{"intentionId":"<id>","search":[...]}'`
...
### 用户输入转义（硬性规则，拼 `--params` 前必须执行）

将用户原文（人名、部门名、昵称等）拼入 `--params '{"..."}'` 的 JSON 字符串前，必须依次完成两层转义，否则会导致 JSON 结构损坏或 shell 命令注入（RCE 风险）。
